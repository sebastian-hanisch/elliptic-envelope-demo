...
- **Merkmalszahl p** bis 30 (Merkmale 13 ... p sind feste Mischungen der Faktoren plus eigenes Rauschen), damit auch der Fall n < p vorkommt.
...
class Dataset:
    ...
    def names(self):
        ...
        return base + [f"Zusatzmerkmal {k}" for k in range(C.N_BASE_FEATURES + 1, self.p + 1)]
...
def generate_dataset(n_tours, p, n_modes, curvature, noise, contamination_pct, kind, strength, seed):
    ...
    if p > C.N_BASE_FEATURES:
        ...
        X = X[:, :p]
    return Dataset(X=X, z=z_used, anomaly=anomaly, mode=mode, n_modes=int(n_modes), kind=kind, p=int(p))
